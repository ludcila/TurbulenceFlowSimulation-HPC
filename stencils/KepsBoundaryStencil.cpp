...
void KepsBoundaryStencil::applyLeftWall ( TurbulentFlowField & flowField, int i, int j, int k ){
	...
	flowField.getDissipationRate().getScalar(i, j, k) =0.09/(0.038*_parameters.geometry.lengthY*(1-_parameters.bfStep.yRatio)*_parameters.geometry.lengthZ*2/(_parameters.geometry.lengthY+_parameters.geometry.lengthZ))*pow(flowField.getKineticEnergy().getScalar(i, j, k),1.5);
